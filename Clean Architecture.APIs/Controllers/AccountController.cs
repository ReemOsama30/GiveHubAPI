...
namespace Clean_Architecture.APIs.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<ActionResult<GeneralResponse>> AdminRegister(AdminRegisterDTO UserRegisterDTO)
        {
            if (ModelState.IsValid)
            {
                IdentityResult result = await _accountService.AdminRegisteration(UserRegisterDTO);
        ...
        [HttpPost("Charity-register")]
        public async Task<ActionResult<GeneralResponse>> CharityRegister(CharityRegisterDTO UserRegisterDTO)
        {
            if (ModelState.IsValid)
            {
                IdentityResult result = await _accountService.CharityRegisteration(UserRegisterDTO);
                ...
                else
                {
                    ...
                    {
                        IsPass = false,
                        Message = "This charity name is already taken",
                        Status = 400
                    };
                }
            }
